...
import time
from openai import OpenAI, RateLimitError, APIConnectionError, APIError, Timeout
import requests
...
class OpenAIBot(Bot, OpenAIImage):
    def __init__(self):
        ...
        if conf().get("open_ai_api_base"):
            client_args["base_url"] = conf().get("open_ai_api_base")
        
        # 处理代理配置
        proxy = conf().get("proxy")
        if proxy:
            client_args["http_client"] = requests.Session()
            client_args["http_client"].proxies = {"http": proxy, "https": proxy}

        self.client = OpenAI(**client_args)
        
        # 模型配置
        self.model = conf().get("model") or "text-davinci-003"
        self.sessions = SessionManager(OpenAISession, model=self.model)
